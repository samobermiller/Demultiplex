...
import bioinfo
import gzip
lane1 = np.zeros((args.l),dtype=np.float64)
print(len(lane1))
with gzip.open(args.f, "rt") as fh:
    ...
    for line in fh:
        ...
        if i%4 == 0:
            ...
            counter_array= (i//4-1) #this line addition for using in numpy since we cant append
...
for i, score in enumerate(lane1):
    #for each score in lane1, index and run:
    mean[i] = np.mean(score)
